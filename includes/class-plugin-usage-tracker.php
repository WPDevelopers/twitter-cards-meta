...
if( ! class_exists( 'Twcm_Plugin_Usage_Tracker') ) {
	...
	class Twcm_Plugin_Usage_Tracker {
		...
		public function optin_notice() {
			...
			if ( stristr( network_site_url( '/' ), 'local' ) !== false || stristr( network_site_url( '/' ), 'localhost' ) !== false || stristr( network_site_url( '/' ), ':8888' ) !== false ) {
				...
				if( $this->marketing != 1 ) {
					...
					$notice_text = __( 'Want to help make <strong>Twitter Cards Meta</strong> even more awesome? You can get a <strong>25% discount coupon</strong> for premium addons if you allow. <a class="twcm-insights-data-we-collect" href="#">What we collect.</a>', 'plugin-usage-tracker' );
				}
				...
				<div class="notice notice-info updated put-dismiss-notice">
					...
					<div class="twcm-insights-data" style="display: none;">
		            ...
		            <?php echo "<script type='text/javascript'>jQuery('.twcm-insights-data-we-collect').on('click', function(e) {
		                    e.preventDefault();
		                    jQuery('.twcm-insights-data').slideToggle('fast');
		                });
		                </script>";?>
				</div>
			<?php
